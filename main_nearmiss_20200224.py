...
QUEUE_MAX_SIZE = 150 # size of cached frames
...
def TriggerEvent(eventSystem, eventType):
    ...
    event_info = eventSystem + ',' + eventType + ',' + str(event_start) + ',' + str(event_end)
    sock = socket.socket()
    sock.settimeout(10)
...
#                         if c[0][-1] > np.mean(c[0]) and c[0][0] < np.mean(c[0]):
				...
				# show gps on frame
                                while(True):
                                    ...
                                    data = line.split(',')
                                    if "GPRMC" in data[0]:
                                        cv2.putText(frame, data[3]+data[4], (500,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 2)
                                        cv2.putText(frame, data[5]+data[6], (500,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 2)
                                        break
                                #cv2.imwrite('./images/'+str(time.ctime())+'.jpg',frame)


        cv2.putText(frame, str(round(1/(time.time() - timeCur),2))+' fps', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 2)
        ...
        if len(uploadFrameQueue) != 0:
            (upload_frame, upload_frame_ID) = uploadFrameQueue.pop(0)
            result, imgencode = cv2.imencode('.jpg', upload_frame, ENCODE_PARAM)
...
#             print('_' + str(len(stringData)).ljust(16) + '_')
...
#             print('UploadFrameQueue length:', len(uploadFrameQueue))
        ...
        if nearmiss == True:
            ...
            # send all the cached frames to upload frame queue
#             print('current frame ID', frame_ID)
#             print('frameIDQueue', frameIDQueue)
#             print('frameIDQueue length', len(frameIDQueue))
#             uploadFrameQueue.ex(list(zip(frameQueue, frameIDQueue)))
        
#         print(eventTrigger)
        if eventTrigger == 1:
#             print(eventTrigger)
            print('Find an event')
            TriggerEvent(eventSystem, eventType)
            eventTrigger = 0
            eventType = ''
            eventSystem = ''
