...
def TriggerEvent(eventSystem, eventType, ttcH, ttcW, slope, gps_str):
    ...
    event_info = eventSystem + ',' + eventType + ',' + str(round(ttcH,2)) + ',' + str(round(ttcW,2)) + ',' + str(round(slope,2)) + ',' + gps_str + ',' + str(event_start) + ',' + str(event_end) + '\n'
...
#    TCP_PORT = 80
    ...
    QUEUE_MAX_SIZE = 60 # size of cached frames
 ...
 #           ret,frame = cap.read()
    ...
    #                         if c[0][-1] > np.mean(c[0]) and c[0][0] < np.mean(c[0]):
            ...
            random_event = (int(datetime.now().strftime('%M'))%15 == 1) & (int(datetime.now().strftime('%S'))<1)
