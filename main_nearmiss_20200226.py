...
def EventMonitor(eventTrigger):
    '''
    Event Monitoring Process.
    '''
    print('Check Event:', eventTrigger)
    while True:
        time.sleep(1)
        if eventTrigger == 1:
            print('Find an event')
...
def TriggerEvent(eventSystem, eventType, ttcH, ttcW, slope, gps):
    ...
    event_info = eventSystem + ',' + eventType + ',' + str(round(ttcH,1)) + ',' + str(round(ttcW,1)) + ',' + str(round(slope,2)) ',' + gps + ',' str(event_start) + ',' + str(event_end)
    sock = socket.socket()
	sock.settimeout(15)
...
def UploadMonitor(uploadFrameQueue):
    '''
    Process of uploading frames. 
    This process will keep reading video frames from the uploadFrameQueue and uploading frames 
    '''
    print(' UploadFrameQueue length:', len(uploadFrameQueue))
    while True:
        if len(uploadFrameQueue) != 0:
            (frame, frame_ID) = uploadFrameQueue.pop()
            print(frame_ID)
...
if __name__ == '__main__':
    ...
    try:
		...
		sock.connect((TCP_IP, TCP_PORT))
		sock.send( str(METADATA_SIZE).ljust(16).encode())
		TELS_start = 'TELS starts running!Message sent.'
		sock.send( TELS_start.ljust(METADATA_SIZE).encode())
		sock.close()
        ...
        frameNum = 0    
    #     # a shared queue (list) in the upload sub-process to store the frame that need to be uploaded to the video storage server
    #     uploadFrameQueue = UploadFrameQueue() 
    #     # video frame upload sub-process
    #     uploadMonitorProcess = mp.Process(target=UploadMonitor, args=(uploadFrameQueue,))
    #     # set process name
    #     uploadMonitorProcess.name = 'UploadMonitorProcess'
    #     # start the video frame upload sub-process
    #     uploadMonitorProcess.start()
        
    
    #     uploadMonitorThread = threading.Thread(target = UploadMonitor, args=(uploadFrameQueue,))
    
    #     uploadMonitorThread.start()
        
        # a share value as a trigger to monitor the event, used in the event monitoring sub-process
    #    eventTrigger = 0
    #    eventType = ''
    #    eventSystem = ''
        # event monitoring event, which keeps monitoring both camera-based and CAN-system-based events
    #     evenMonitorThread = threading.Thread(target=EventMonitor, args=(eventTrigger,))
     
        # start the event monitoring event
    #     evenMonitorThread.start()
    ...
    #                         if c[0][-1] > np.mean(c[0]) and c[0][0] < np.mean(c[0]):
    				...
    				# show gps on frame
                                    while(True):
                                        ...
                                        data = line.split(',')
                                        if "GPRMC" in data[0]:
											gps = data[3]+data[4]+','+data[5]+data[6]
                                            cv2.putText(frame, data[3]+data[4], (280,40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 1)
                                            cv2.putText(frame, data[5]+data[6], (280,60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 1)
                                            break
                                    #cv2.imwrite('./images/'+str(time.ctime())+'.jpg',frame)
    
    
            cv2.putText(frame, str(round(1/(time.time() - timeCur),2))+' fps', (40,40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [0,255,255], 1)
            ...
            # simulating to trigger an event
            if nearmiss == True:
                
                # trigger an event               
                print('Trigger an event.')
    ...
    #             print(eventTrigger)
                print('Find an event')
                TriggerEvent(eventSystem, eventType, ttcH, ttcW, slope, gps)
    #            eventTrigger = 0
    #            eventType = 'N/A'
    #            eventSystem = 'N/A'
